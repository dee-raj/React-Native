...
import { getDifficultyList, getCryptogramLevels, STORAGE_KEY, getTotalLevels } from './CryptogramConfig';

const { width } = Dimensions.get('window');

const CryptogramLevelSelectionScreen = ({ navigation }) => {
    const [completedLevels, setCompletedLevels] = useState({ easy: [], classic: [], hard: [] });
        ...
        const load = async () => {
            try {
                ...
                if (saved) {
                    ...
                    setCompletedLevels(data.completedLevels || { easy: [], classic: [], hard: [] });
                }
            } catch (e) {
                console.error('Failed to load cryptogram progress', e);
            }
            setLoaded(true);
        };
    ...
    useEffect(() => {
        if (loaded && animValues.current.length > 0) {
            Animated.stagger(60,
                animValues.current.map(anim =>
                    anim ? Animated.timing(anim, { toValue: 1, duration: 350, useNativeDriver: true }) : null
                ).filter(Boolean)
            ).start();
        }
    }, [loaded]);
    ...
    const isSectionUnlocked = (difficultyId) => {
        ...
        if (difficultyId === 'classic') {
            return (completedLevels.easy || []).length >= getTotalLevels('easy');
        }
        if (difficultyId === 'hard') {
            return (completedLevels.classic || []).length >= getTotalLevels('classic');
        }
        return false;
    };
    ...
    difficulties.forEach(diff => {
        const levels = getCryptogramLevels(diff.id);
        levels.forEach(lvl => {
            if (!animValues.current[itemIndex]) animValues.current[itemIndex] = new Animated.Value(0);
        ...
        return (
            <SafeAreaView style={styles.safeArea}>
                <LinearGradient colors={['#1a1a2e', '#16213e', '#0f3460']} style={[styles.container, styles.centered]}>
                    <Ionicons name="key-outline" size={48} color="rgba(255,255,255,0.3)" />
                </LinearGradient>
            </SafeAreaView>
        );
    }
        ...
        <SafeAreaView style={styles.safeArea}>
            <LinearGradient colors={['#1a1a2e', '#16213e', '#0f3460']} style={styles.container}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.title}>Cryptogram</Text>
                    <View style={{ width: 44 }} />
                </View>

                <ScrollView>
                    <Text style={styles.subtitle}>Decode the Quotes</Text>
                                ...
                                <View style={styles.sectionHeader}>
                                    ...
                                    {!sectionUnlocked && (
                                        <Text style={styles.lockedHint}>
                                            {diff.id === 'classic' ? 'Complete all Easy levels' : 'Complete all Classic levels'}
                                        </Text>
                                    )}
                                </View>

                                <View style={[styles.levelsRow, !sectionUnlocked && styles.sectionLocked]}>
                                    {getCryptogramLevels(diff.id).map((lvl) => {
                                        ...
                                        return (
                                            <Pressable
                                                ...
                                                onPress={() => {
                                                    if (!isLocked) {
                                                        navigation.navigate('CryptogramGame', {
                                                            difficulty: diff.id,
                                                            level: lvl.index,
                                                        });
                                                    }
                    ...
                    <View style={styles.rulesContainer}>
                        <Text style={styles.rulesTitle}>🔐 Cryptogram</Text>

                        <Text style={styles.rulesDescription}>
                            Decode famous quotes by figuring out the letter substitution cipher.
                        </Text>

                        <View style={styles.ruleItem}>
                            <Text style={styles.bullet}>•</Text>
                            <Text style={styles.ruleText}>Each letter is replaced by another letter</Text>
                        </View>

                        <View style={styles.ruleItem}>
                            <Text style={styles.bullet}>•</Text>
                            <Text style={styles.ruleText}>Tap a cipher letter, then choose the decoded letter</Text>
                        </View>

                        <View style={styles.ruleItem}>
                            <Text style={styles.bullet}>•</Text>
                            <Text style={styles.ruleText}>Use hints wisely — they reveal one correct letter</Text>
                        </View>

                        <View style={styles.ruleItem}>
                            <Text style={styles.bullet}>•</Text>
                            <Text style={styles.ruleText}>Wrong guesses cost lives — don't run out!</Text>
                        </View>
                    </View>
                </ScrollView>
            </LinearGradient>
...
export default CryptogramLevelSelectionScreen;
